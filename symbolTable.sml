...
fun mkLocals globals (FUNCTION {params=params, decls=decls, ...}) =
    let
        val ht = copy globals
    in
        List.app checkTypeExist decls;
...
fun addLocals locals globals (func as FUNCTION {id=id, ...}) =
    insert locals (id, mkLocals globals func)


(*Check if the type is valid. If so, add it to the globals table*)
fun addGlobal globals (vd as VAR_DECL {id=id, typ=t, ...}) =
    (checkTypeExist vd; insert globals (id, t))
...
fun mkSymbolTable file (PROGRAM {types=ts, decls=ds, funcs=fs}) =
    ...
    in
        List.app addTypeDecl ts;
        List.app (addGlobal globals) ds
        ...
        List.app (addLocals locals globals) fs;
        ST {types=copy types, globals=globals, funcs=funcs, locals=locals}
    end
end
